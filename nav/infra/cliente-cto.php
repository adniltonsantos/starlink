...
if (isset($_GET['comentario'])){
    ...
    echo "<script>location.href='?pg=cliente-cto</script>";  
}
?>
...
if (isset($_GET['cancelou'])){
    ...
    echo "<script>alert('Cliente Cancelado com Sucesso'); location.href='?pg=cliente-cto'</script>"; 
}
?>
...
if (isset($_GET['retornar'])){
    ...
    $instalasql = $pdo->prepare("UPDATE instalacoes SET status_agendamento='retornocto' WHERE id_instalacao='$id_instalacao'");
    $instalasql->execute();

    $cancelasql = $pdo->prepare("UPDATE clientes SET status_cliente='retornocto' WHERE id_cliente='$id_cliente'");
    $cancelasql->execute();
    
    echo "<script>alert('Cliente Retornado com Sucesso'); location.href='?pg=cliente-cto'</script>"; 
}
?>

<legend>Clientes parados por causa de CTO</legend>
...
<!-- Fecha Formulario-->
       ...
       <?php 
        ...
        <td class="centro-table"><a href="?pg=cliente-cto&retornar&id_instalacao=<?php echo $linha['id_instalacao'];?>&id_cliente=<?php echo $linha['id_cliente'];?>"><div  onclick="if (! confirm('Deseja Retornar a instalacao para o agendamento COD - <?php echo $linha['cod_cliente']; ?>')) { return false; }"class="glyphicon glyphicon-retweet" title="Retornar" data-toggle="tooltip"></div></td>
        <td class="centro-table"><a href="?pg=cliente-cto&cancelou&id_instalacao=<?php echo $linha['id_instalacao'];?>&id_cliente=<?php echo $linha['id_cliente'];?>"><div  onclick="if (! confirm('Deseja Cancelar a instalação, COD - <?php echo $linha['cod_cliente']; ?>')) { return false; }"class="glyphicon glyphicon-remove" title="Cancelou" data-toggle="tooltip"></div></td>
...
<?php echo $linha['cod_cliente']?> - <?php echo $linha['nomeCliente']?>
     ...
     <div style="padding:20px">
      ...
      <form method="POST" id="comentario<?php echo $linha['id_cliente'];?>" action="?pg=cliente-cto&comentario">
