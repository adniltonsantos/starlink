...
if (isset($_GET['cancelou'])){
    ...
    echo "<script>alert('Cliente Cancelado com Sucesso'); location.href='?pg=clientes-hack'</script>"; 
}
?>
...
if (isset($_GET['comentario'])){
    ...
    echo "<script>location.href='?pg=clientes-hack</script>";  
}
?>
...
<!-- Fecha Formulario-->
       ...
       <?php 
        ...
        $agendadosql = $pdo->prepare("SELECT *, nome as nomeCliente from clientes WHERE status_cliente='aguardando-instalacaoHack' ORDER BY data_cadastro ASC ");
        ...
        <td class="centro-table"><a href="?pg=clientes-hack&cancelou&id_cliente=<?php echo $linha['id_cliente'];?>"><div  onclick="if (! confirm('Deseja Cancelar o cliente com o código , <?php echo $linha['cod_cliente']; ?>')) { return false; }"class="glyphicon glyphicon-remove" title="Cancelou" data-toggle="tooltip"></div></td>
...
<?php echo $linha['cod_cliente']?> - <?php echo $linha['nome']?>
      ...
      <form method="POST" id="agendar<?php echo $linha['id_cliente'];?>" action="?pg=clientes-hack&update">
