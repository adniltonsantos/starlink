...
if (isset($_GET['cancelou'])){
    ...
    echo "<script>alert('Cliente Cancelado com Sucesso'); location.href='?pg=clientes-whats'</script>"; 
}
?>
...
if (isset($_GET['comentario'])){
    ...
    echo "<script>location.href='?pg=clientes-whats</script>";  
}
?>
...
<!-- Fecha Formulario-->
       ...
       <?php 
        ...
        $agendadosql = $pdo->prepare("SELECT *, nome as nomeCliente from clientes WHERE status_cliente='whats' ORDER BY data_cadastro ASC ");
        ...
        <td class="centro-table"><a href="?pg=clientes-whats&id_cliente=<?php echo $linha['id_cliente'];?>"><div aria-hidden="true" data-toggle="modal" data-target="#myModal<?php echo $linha['id_cliente']?>" class="glyphicon glyphicon-time"></div></td>
        ...
        <td class="centro-table"><a href="?pg=clientes-whats&semcontato&id_cliente=<?php echo $linha['id_cliente'];?>"><div data-target="#myModal<?php echo $linha['id_cliente']?>" class="glyphicon glyphicon-erase" title="Sem Contato" data-toggle="tooltip"></div></td>
        <td class="centro-table"><a href="?pg=clientes-whats&cancelou&id_cliente=<?php echo $linha['id_cliente'];?>"><div  onclick="if (! confirm('Deseja Cancelar o cliente com o código , <?php echo $linha['cod_cliente']; ?>')) { return false; }"class="glyphicon glyphicon-remove" title="Cancelou" data-toggle="tooltip"></div></td>
        </tr>
...
<?php echo $linha['cod_cliente']?> - <?php echo $linha['nome']?>
      ...
      <form method="POST" id="agendar<?php echo $linha['id_cliente'];?>" action="?pg=clientes-whats&update">
...
<?php echo $linha['cod_cliente']?> - <?php echo $linha['nomeCliente']?>
     ...
     <div style="padding:20px">
      ...
      <form method="POST" id="comentario<?php echo $linha['id_cliente'];?>" action="?pg=clientes-whats&comentario">
